...
%% Parameters:
    theta   = 50;                   % direction of arrival (degrees)
    ...
    alpha   = pi/M;                 % angle where the third sensor is located (pi/3 for equilateral triangle)
...
%% Beam pattern:
    ...
    for th = 1:N_theta
        for i = 1:N_D            
            aUTA_theta      = [1 exp(2j*pi*D_i(i)/lambda*sin(doa_theta(th)- doa)) exp(2j*pi*D_i(i)/lambda*sin(doa_theta(th)- doa+alpha))]';
            %aUTA_theta      = exp(2j*pi*D_i(i)/lambda*sin(doa_theta(th) - doa)*m); % general ULA vector       
            G_thetaD(th, i) = w'*aUTA_theta;
        end
        
        for i = 1:N_F
            aUTA_theta      = [1 exp(2j*pi*D/(c/f_i(i))*sin(doa_theta(th)- doa)) exp(2j*pi*D/(c/f_i(i))*sin(doa_theta(th)- doa+alpha))]';
            
            %aUTA_theta      = exp(2j*pi*D/(c/f_i(i))*sin(doa_theta(th) - doa)*m); % general ULA vector       
            G_thetaF(th, i) = w'*aUTA_theta;
        end
    end 
