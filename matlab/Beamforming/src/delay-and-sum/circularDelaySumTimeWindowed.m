...
theta1  = 90;                   % 1st direction of arrival (degrees)
...
M       = 6;                    % number of sensors (fixed to 3 for a triangular beaformer)
alpha   = [0 pi/3 pi/6 0 -pi/6 -pi/3];
radius  = lambda(k)/2;
D       = 2*radius;               % separation of consecutive sensors (meters)
...
aUCA        = w.*[1 exp(2j*pi*d*cos(alpha(2))*sin(doa(k)-alpha(2))) ...
                    exp(2j*pi*d*cos(alpha(3))*sin(doa(k)-alpha(3))) ...
                    exp(2j*pi*d*cos(alpha(4))*sin(doa(k)-alpha(4))) ...
                    exp(2j*pi*d*cos(alpha(5))*sin(doa(k)-alpha(5))) ...
                    exp(2j*pi*d*cos(alpha(6))*sin(doa(k)-alpha(6))) ]';       % ULA steering vector

% ULA weight vector (with unit length):
wUCA  = aUCA/sqrt(aUCA'*aUCA);

%% Sensor outputs:
X = zeros(M,N);
X(1,:) = s1+s2;
delta1 = [0 D*cos(alpha(2))*sin(doa(1)-alpha(2)) ...
            D*cos(alpha(3))*sin(doa(1)-alpha(3)) ...
            D*cos(alpha(4))*sin(doa(1)-alpha(4)) ...
            D*cos(alpha(5))*sin(doa(1)-alpha(5)) ...
            D*cos(alpha(6))*sin(doa(1)-alpha(6)) ]/c;
        
delta2 = [0 D*cos(alpha(2))*sin(doa(2)-alpha(2)) ...
            D*cos(alpha(3))*sin(doa(2)-alpha(3)) ...
            D*cos(alpha(4))*sin(doa(2)-alpha(4)) ...
            D*cos(alpha(5))*sin(doa(2)-alpha(5)) ...
            D*cos(alpha(6))*sin(doa(2)-alpha(6)) ]/c;        
...
Pconv1 = real( (aUCA'*Rx*aUCA) / (aUCA'*aUCA) );
Pconv2 = real( wUCA'*Rx*wUCA );
Pconv3 = abs(Y0)*abs(Y0')/N;

%% Beam pattern:
N_theta     = 1000;
doa_theta   = linspace(-pi, pi, N_theta);       % general DOA vector 
G_theta     = zeros(N_theta, 1);
for th = 1:N_theta
    exp_theta =  [1 exp(2j*pi*d*cos(alpha(2))*sin(doa_theta(th)-alpha(2))) ...
                    exp(2j*pi*d*cos(alpha(3))*sin(doa_theta(th)-alpha(3))) ...
                    exp(2j*pi*d*cos(alpha(4))*sin(doa_theta(th)-alpha(4))) ...
                    exp(2j*pi*d*cos(alpha(5))*sin(doa_theta(th)-alpha(5))) ...
                    exp(2j*pi*d*cos(alpha(6))*sin(doa_theta(th)-alpha(6))) ]; 

    aUCA_theta = w.*exp_theta';
    %aULA_theta = w.*exp(2j*pi*d*sin(doa_theta(th))*m); % general ULA vector
    G_theta(th) = aUCA'*aUCA_theta/M;
