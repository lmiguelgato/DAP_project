...
aUTA        = w.*[1 exp(2j*pi*d*sin(doa(k)+alpha)) exp(2j*pi*d*sin(doa(k)))]';       % ULA steering vector
...
X = zeros(M,N);
X(1,:) = s1+s2;
delta1 = [0 D*sin(doa(1)+alpha) D*sin(doa(1))]/c;
delta2 = [0 D*sin(doa(2)+alpha) D*sin(doa(2))]/c;
for m = 1:M-1
...
for th = 1:N_theta
    aUTA_theta = w.*[1 exp(2j*pi*d*sin(doa_theta(th)+alpha)) exp(2j*pi*d*sin(doa_theta(th)))]';
