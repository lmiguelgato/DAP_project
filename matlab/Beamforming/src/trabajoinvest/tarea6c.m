...
% Load sample audio (two channels) and load impulse response of the room
...
%% Parameters:
    ...
    %% Load audio file
    [filename, path] = uigetfile( {'*.wav;','Audio files (*.wav)'; },'Pick impulse response');
    if isequal(filename,0)
       disp('The user cancelled the audio loading operation.')
       return;
    else
       [hn,fs] = audioread(strcat((path),(filename)));      
    end
