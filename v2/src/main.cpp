/*
 * Multiple audio sources localization and tracking using a triangular array of microphones.
 *
 * Additionally, a simple delay-and-sum beamformer allows separation of audio sources.
 *
 * Author: 		Luis Miguel Gato Diaz 		e-mail: lmiguelgato@gmail.com
 * Supervisor:	Caleb Rascon Estebane		e-mail: caleb.rascon@gmail.com
 */
...
int n_out_channels = 2;		// number of output channels
int n_in_channels = 3;			// number of input channels
...
int jack_callback (jack_nframes_t nframes, void *arg){
	...
	for (j = 0; j < n_in_channels; ++j) {
		...
		for (int i = nframes; i < window_size_2; ++i)
			i_time_2N[i] = 0.0;

		// 2- apply FFT:
		fftw_execute(i_forward_2N);

		for (int i = 0; i < window_size_2; ++i)
	...
	doa = angleRedundancy (theta, REDUNDANCY_TH);
